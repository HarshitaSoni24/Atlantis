...
interface RiskLegendProps {
  riskLevels: { label: string; key: 'mouth' | 'low' | 'moderate' | 'light' | 'high' }[];
}
    ...
    <Paper sx={{
      ...
      {riskLevels.map((level) => (
        <Box key={level.label} sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
          <Box sx={{ width: 20, height: 20, bgcolor: theme.palette.custom.risk[level.key], mr: 1, border: '1px solid #ccc' }} />
